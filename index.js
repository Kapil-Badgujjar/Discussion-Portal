let formContainer = document.getElementById("form-side");
let addNewQuestionButton = document.getElementById("new-question-button");
let x = [],y=[],F=[];
let QKey=0;
addNewQuestionButton.addEventListener("click", openAddNewQuestionForm);

function sorByVotes(){
    x.sort((a,b) =>{
         if(b.favorite==true&&a.favorite==true) return b.Votes-a.Votes;
         else if(b.favorite==false&&a.favorite==false) return b.Votes-a.Votes;
...
function setTimeDifference(obj,pB2){
    ...
    if(dyear>=2) pB2.innerHTML=dyear+" Years ago";
        ...
        if(dmonth>=0) pB2.innerHTML="1 Year ago";
        else pB2.innerHTML=(12+dmonth) + " Months ago";
    }
    else if(dmonth>=2) pB2.innerHTML=dmonth+" Month ago";
    else if(dmonth==1){
        if(dDD>=0) pB2.innerHTML= "1 Month ago";
        else pB2.innerHTML= (30+dDD) + " Days ago";
    }
    else if(dDD>=2) pB2.innerHTML= dDD+" Days ago";
    else if(dDD==1){
        if(dHH>=0) pB2.innerHTML= "1 Day ago";
        else pB2.innerHTML= (24+dHH) + " Hours ago";
    }
    else if(dHH>=2) pB2.innerHTML= dHH+" Hours ago";
    else if(dHH==1){
        if(dMM>=0) pB2.innerHTML= "1 Hour ago";
        else pB2.innerHTML= (60+dMM) + " Minutes ago";
    }
    else if(dMM>=2) pB2.innerHTML= dMM+" Minutes ago";
    else if(dMM==1){
        if(dSS>=0) pB2.innerHTML= "1 Minute ago";
...
function openQuestionDescription(obj,key) {
    ...
    tempResolveButton.addEventListener("click",function(){
        let d = searchObjectById(obj.qid);
        if(d!=undefined){
            x.splice(d,1);
            localStorage.setItem("DiscussionAppData",JSON.stringify(x));
            refreshList(x);
...
function createNewResponse(){
    ...
    if(userName==""||userAnswer=="") { alert("Please enter userName and answer!"); return}
    let obj = {
        name: userName,
        answer: userAnswer
    };
...
function openAddNewQuestionForm() {
    ...
    tempP.innerHTML = "Enter a subject and question to get started";
    ...
    tempTextarea.setAttribute("placeholder","Type your question here : ");
    ...
    let tempButton = document.createElement("button");
    tempButton.innerHTML = "Submit";
    tempButton.addEventListener("click", addNewQuestion)
    tempDiv2.appendChild(tempButton);
    formContainer.appendChild(tempDiv2);
}
function addNewQuestion(){
    let titleBox = document.getElementById("questionTitle");
    let questionBox=document.getElementById("askedQuestion");
    let title=titleBox.value.trim();
    let question=questionBox.value.trim();
    if(title==""||question==""){ alert("Please enter question title and question properly!"); return;}
